    ...
    try {
      const response = await fetch('/api/musicas', {
        ...
        body: JSON.stringify({
          ...
          links_youtube: formData.links_youtube
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              onClick={() => setFormData(prev => ({
                ...prev,
                links_youtube: [...prev.links_youtube, { url: '', titulo: '' }]
              }))}
              className="px-3 py-1.5 bg-primary text-white rounded-lg hover:bg-primary-light transition-colors text-sm font-semibold"
            >
              + Adicionar Link
            </button>
          </div>
          {formData.links_youtube.map((link, index) => (
              ...
              <div className="flex gap-2 mb-2">
                <div className="flex-1">
                  <input
                    ...
                    value={link.titulo}
                    onChange={(e) => {
                      const newLinks = [...formData.links_youtube]
                      newLinks[index].titulo = e.target.value
                      setFormData(prev => ({ ...prev, links_youtube: newLinks }))
                    }}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    placeholder="Título (opcional, ex: Versão original, Cover, etc.)"
                  />
                </div>
                {formData.links_youtube.length > 1 && (
                  <button
                    type="button"
                    onClick={() => {
                      const newLinks = formData.links_youtube.filter((_, i) => i !== index)
                      setFormData(prev => ({ ...prev, links_youtube: newLinks }))
                    }}
