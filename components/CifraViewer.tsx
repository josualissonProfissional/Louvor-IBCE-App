...
// Componente para visualização de cifra com controles de acessibilidade
import { useEffect, useState } from 'react'
import { useParams } from 'next/navigation'
import Link from 'next/link'
import { createClient } from '@/lib/supabase/client'
import { transposeCifra } from '@/lib/cifra-transposer'

// Função auxiliar para transpor uma nota (extraída da lib)
// Sempre usa notação de sustenidos (#) ao invés de bemóis (b)
function transposeNote(note: string, semitones: number): string {
  ...
  // Sempre usa a notação de sustenidos (#) ao invés de bemóis (b)
...
// Gera array de semitons de -12 a +12 (uma oitava completa em cada direção)
const SEMITONES = Array.from({ length: 25 }, (_, i) => i - 12)

// Função para gerar label do semiton
const getSemitoneLabel = (semitone: number): string => {
  if (semitone === 0) return 'Original'
  if (semitone > 0) return `+${semitone} (${semitone} meio(s) tom acima)`
  return `${semitone} (${Math.abs(semitone)} meio(s) tom abaixo)`
}
...
export default function CifraViewer() {
  ...
  const [textColor, setTextColor] = useState('#F97316') // Laranja como padrão
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-gray-900 dark:text-white">Carregando...</div>
      </div>
    )
  }

  if (!musica || !musica.cifras || musica.cifras.length === 0) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-gray-900 dark:text-white">Cifra não encontrada.</div>
      </div>
    )
  }
  ...
  const transposedCifra = semitones !== 0
    ? transposeOnlyChords(cifra.texto, semitones)
    : cifra.texto
  ...
  const renderCifraWithColoredChords = (text: string): string | (string | JSX.Element)[] => {
    if (textColor === 'default') {
      // Se for padrão, não precisa aplicar cor especial
      return text
    }
    ...
    lines.forEach((line, lineIndex) => {
      ...
      // Linha de acordes geralmente tem: poucas letras minúsculas, muitos espaços, acordes isolados
      const lowercaseCount = (line.match(/[a-z]/g) || []).length
      const uppercaseCount = (line.match(/[A-G]/g) || []).length
      const spaceCount = (line.match(/\s/g) || []).length
      const isLikelyChordLine = uppercaseCount > lowercaseCount && spaceCount > 0
      ...
      while ((match = chordPattern.exec(line)) !== null) {
        ...
        // Verifica se está em contexto de acorde
        const isStartOfLine = matchIndex === 0
        const isAfterWhitespace = /[\s\t\[\(]/.test(charBefore)
        const isBeforeWhitespace = /[\s\n\r\t\]\)]/.test(charAfter) || matchIndex + possibleChord.length === line.length
        const isBeforeSlash = charAfter === '/' // Nota de baixo (ex: C/E)
        
        // Verifica se há letras minúsculas antes e depois (indica que é parte de palavra)
        const hasLowercaseBefore = matchIndex > 0 && /[a-z]/.test(charBefore)
        const hasLowercaseAfter = /[a-z]/.test(charAfter) && !isBeforeWhitespace && !isBeforeSlash
        
        // Verifica contexto maior: se há letras minúsculas formando palavras próximas
        const contextBefore = line.substring(Math.max(0, matchIndex - 2), matchIndex)
        const contextAfter = line.substring(matchIndex + possibleChord.length, Math.min(line.length, matchIndex + possibleChord.length + 3))
        const hasWordContext = /[a-z]{2,}/.test(contextBefore + contextAfter)
        
        // Verifica se está dentro de uma palavra (letras minúsculas antes e depois sem espaço)
        const isInWord = hasLowercaseBefore && hasLowercaseAfter && !isBeforeWhitespace
        
        // É acorde se:
        // 1. Está em uma linha que parece ser de acordes (muitos acordes, poucas letras minúsculas) OU
        // 2. Está isolado (não parte de palavra) com verificações rigorosas
        const isIsolatedChord = !isInWord &&
                                !hasWordContext &&
                                (isStartOfLine || isAfterWhitespace) && 
                                (isBeforeWhitespace || isBeforeSlash) && 
